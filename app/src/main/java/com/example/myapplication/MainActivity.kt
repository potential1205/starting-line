...
import com.example.myapplication.service.MarathonApiService
...
class MainActivity : ComponentActivity(), DataClient.OnDataChangedListener {
    ...
    private var serverBaseUrl = "http://192.168.24.246:8080/api/v1/watch/"
        ...
        apiService.startMarathon(marathonId, userId).enqueue(object : Callback<MarathonReadyDto> {
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    Log.d("leejaehoon", "Marathon Ready 성공: ${response.body()}")
                    marathonReadyData.value = response.body()
                    isMarathonReady.value = true;
                    sendMarathonReady();
                } else Log.e("leejaehoon", "Marathon Ready 응답 에러 ${response.errorBody()?.string()}")
            }

            override fun onFailure(call: Call<MarathonReadyDto>, t: Throwable) {
                Log.e("leejaehoon", "Marathon Ready 호출 실패", t)
            }
        })
    }
            ...
            if (event.type == DataEvent.TYPE_CHANGED) {
                ...
                when (dataItem.uri.path) {
                    "/start" -> { // 워치로 부터 /start 요청이 오면
                        ...
                        Log.d("leejaehoon", "Marathon Start 성공: ")
                    }
                    "/end" -> { // 워치로 부터 /end 요청이 오면
                        isAutoSending.value = false
                        runOnUiThread {
                            Log.d("leejaehoon", "Marathon End 성공: ")
                        }
                    }
                }
            }
    ...
    private fun sendDataToWearable(data: MyData) {
        ...
        Wearable.getDataClient(this).putDataItem(putDataRequest)
            .addOnSuccessListener { dataItem ->
                Log.e("leejaehoon", "Marathon 데이터 전송 성공: $dataItem")
            }
            .addOnFailureListener { e ->
                Log.e("leejaehoon", "Marathon 데이터 전송 실패", e)
            }
    }
    ...
    private fun sendMarathonReady() {
        ...
        Wearable.getDataClient(this).putDataItem(putDataRequest)
            .addOnSuccessListener { dataItem ->
                Log.e("leejaehoon", "Marathon Ready 전송 성공: $dataItem")
            }
            .addOnFailureListener { e ->
                Log.e("leejaehoon", "Marathon Ready 전송 실패", e)
            }
    }
